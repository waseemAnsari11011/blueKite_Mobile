...
import { fetchNewArrivalVendors } from '../../config/redux/actions/vendorActions';

const NewArrivalVendorsScreen = ({ navigation }) => {
  const dispatch = useDispatch();
  const { newArrivalVendors, loading } = useSelector(state => state.vendors);
  ...
  useEffect(() => {
    if (data?.user?.shippingAddresses?.location?.coordinates) {
        ...
        dispatch(fetchNewArrivalVendors(lat, long));
    } else {
        dispatch(fetchNewArrivalVendors());
    }
  }, [dispatch, data]);
...
export default NewArrivalVendorsScreen;
