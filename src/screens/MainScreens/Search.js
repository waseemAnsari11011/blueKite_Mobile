...
import {
  ...
  resetSearchResults,
  updateSearchQuery,
} from '../../config/redux/actions/searchActions';
...
const SearchScreen = ({navigation}) => {
  ...
  useEffect(() => {
    if (query) {
      dispatch(searchProducts(query, 1, 10, userAddress));
    }
    return () => {
  ...
  const renderItem = ({item, index}) => {
    return (
      <View
        style={[
          index === products.length - 1 && cartItems.length > 0
            ? styles.lastItem
            : index === products.length - 1 && {marginBottom: 15},
